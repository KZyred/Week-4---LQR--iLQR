...
def change_dt(dt):
    DELTA_T = dt
    print("change_dt: ", DELTA_T)
def get_next_state(z,u):
    """
    Inputs:
    1. z: state of the quadrotor as a numpy array (x, vx, y, vy, theta, omega)
    2. u: control as a numpy array (u1, u2)

    Output:
    1. the new state of the quadrotor as a numpy array
    """
    x = z[0]
    vx = z[1]
    y = z[2]
...
def simulate(z0, controller, horizon_length, disturbance = False):
    ...
    Inputs:
    1. z0: the initial conditions of the quadrotor as a numpy array (x, vx, y, vy, theta, omega)
    2. controller: a function that takes a state z as argument and index i of the time step and returns a control u
    3. horizon_length: the horizon length

    4. disturbance: if True will generate a random push every seconds during the simulation

    Output:
    1. t[time_horizon+1] contains the simulation time
    2. z[4, time_horizon+1] : contains the time evolution of states
    3. u[2, time_horizon] : contains the time evolution of control
    """
...
def animate_robot(x, u, dt = DELTA_T):
    """
    ...
    plt.close(fig)
    plt.close(ani._fig)
    IPython.display.display_html(IPython.core.display.HTML(ani.to_html5_video()))       # lâu ở đây
