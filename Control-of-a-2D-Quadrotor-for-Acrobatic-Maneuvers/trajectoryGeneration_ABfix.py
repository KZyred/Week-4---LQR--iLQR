...
class solver3:
    def __init__(self, horizon_length = 1000):
        ...
        self.K, self.k = self.solve_LQR_trajectory(A, B, self.move_in_a_circle)
        ...
        self.K_tilted, self.k_tilted = self.solve_LQR_trajectory(A, B, self.move_in_a_circle_tilted)
    ...
    def move_circular_controller(self, state, i):
        return (self.K[i] @ state) + self.k[i] + np.array([self.m * self.g / 2, self.m * self.g / 2])

    def move_circular_tilted_controller(self, state, i):
        return (self.K_tilted[i] @ state) + self.k_tilted[i] + np.array([self.m * self.g / 2, self.m * self.g / 2])
